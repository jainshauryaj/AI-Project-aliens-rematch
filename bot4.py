...
Initialize the game with two crew members.
Initialize the probabilities for the positions of the alien and the two crew members.
Run the game for a certain number of timesteps.
    At each timestep:
        Update the probabilities based on the bot's sensors.
        Choose the next move based on the probabilities.
        Move the bot.
        If the bot's position is the same as a crew member's position, teleport that crew member away and update the probabilities accordingly.
        Print the current state.
Repeat until both crew members have been found.
